        ...
        <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div class="space-y-2">
                ...
                <p class="max-w-2xl text-sm leading-6 text-zinc-600 dark:text-zinc-400">
                    {{ __('Update the listing for :company. Ownership stays tied to your company on the server.', ['company' => $companyName]) }}
                </p>
            </div>
    ...
    <form wire:submit="save" class="space-y-6">
            ...
            <div class="grid gap-4 md:grid-cols-2">
                ...
                <div class="md:col-span-2 space-y-4">
                    ...
                    @if (count($existingImages) > 0)
                        <div class="space-y-3">
                            <div class="flex items-center justify-between gap-3">
                                ...
                                <p class="text-sm text-zinc-500 dark:text-zinc-400">{{ __('Remove an image to delete it immediately.') }}</p>
                            </div>
                                ...
                                @foreach ($existingImages as $image)
                                    <figure wire:key="existing-image-{{ $image['id'] }}" class="overflow-hidden rounded-2xl border border-zinc-200 bg-zinc-50 shadow-sm dark:border-zinc-700 dark:bg-zinc-950/40">
                                        ...
                                        <figcaption class="flex items-center justify-between gap-3 px-3 py-2">
                                            <span class="truncate text-sm font-medium text-zinc-700 dark:text-zinc-300">{{ $image['name'] }}</span>

                                            <button type="button" wire:click="removeExistingImage({{ $image['id'] }})" class="text-sm font-medium text-red-600 hover:text-red-700">
                                                {{ __('Remove') }}
                                            </button>
                                        </figcaption>
                                    </figure>
                    ...
                    @if (count($images) > 0)
                        <div class="space-y-3">
                            <div class="flex items-center justify-between gap-3">
                                <h2 class="text-sm font-semibold uppercase tracking-[0.2em] text-zinc-500 dark:text-zinc-400">{{ __('New images') }}</h2>
                                <p class="text-sm text-zinc-500 dark:text-zinc-400">{{ __('These will be added when you save the listing.') }}</p>
                            </div>
        ...
        <div class="rounded-2xl border border-red-200 bg-red-50 p-5 shadow-sm dark:border-red-900/50 dark:bg-red-950/20">
            <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    <h2 class="text-base font-semibold text-red-900 dark:text-red-100">{{ __('Delete listing') }}</h2>
